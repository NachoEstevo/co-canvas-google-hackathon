...
export class CustomAssetStore implements TLAssetStore {
  ...
  resolve(asset: TLAsset, ctx: TLAssetContext): string | Promise<string | null> | null {
    ...
    if (asset.type === 'image' && asset.props.src) {
      // Handle R2 URLs - use proxy for all R2 URLs until CORS is properly configured
      const isR2Url = asset.props.src.includes('idea-fusion.0236038de169a8251e3492c5d72e7d02.r2.cloudflarestorage.com') || 
                      asset.props.src.includes('pub-bf93ed9de7d9453885219b237362e9e7.r2.dev')
